...
class Trainer:
    ...
    def lrrt(self, freeze_pretrained_layers: bool):
        ...
        for decay_it in range(self.lrrt_max_decays + 1):
            ...
            for candidate_id, lr_candidate in enumerate(candidate_lrs):
                module = torch.load(self.checkpoint_running)
                adam = Adam(params=module.parameters(), lr=lr_candidate)
                candidate_slopes[candidate_id] = self.train_n_batches(module=module, adam=adam, n_batches=self.lrrt_n_batches, freeze_pretrained_layers=freeze_pretrained_layers)[1]
    ...
    def train_n_batches(self, module: Module, adam: Adam, n_batches: int, freeze_pretrained_layers: bool):
        ...
        for train_iter, batch in enumerate(self.loader_train):
            ...
            losses.append(self.train_batch(module=module, adam=adam, batch=batch, freeze_pretrained_layers=freeze_pretrained_layers))
        ...
        """
        x_plot = np.arange(len(losses))
        y_polyfit = [bias_total + i * slope_total for i in range(len(losses))]
        plt.scatter(x=x_plot, y=losses, color="black", alpha=0.4, label="batch losses")
        plt.plot(x_plot, y_polyfit, color="red", alpha=0.6,  label="polyfit")
        plt.legend()
        plt.title("loss for training " + str(n_batches) + " iterations with lr " + str(adam.param_groups[0]["lr"]))
        plt.show()
        """
    ...
    def train_n_epochs_early_stop_initial_lrrt(self, n_epochs: int, freeze_pretrained_layers: bool):
        ...
        for epoch in range(1, n_epochs+1):
            ...
            self.train_n_batches(module=module, adam=adam, n_batches=len(self.loader_train), freeze_pretrained_layers=freeze_pretrained_layers)
